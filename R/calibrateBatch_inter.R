...
calibrateBatch.inter.rlm <- function (data = ..., feature = feature, batch = batch, intensity = intensity,
                                      group = ...)
{
...
calibrateBatch.inter.rlm.group <- function(data = ...,
  ...
  data_n <- data %>%
    dplyr::group_by(!! batch, !!feature, !!group_factor) %>% tidyr::nest() %>% # very serious error could happen if i am wrong with this line
    dplyr::mutate(data_intra_calibrate = purrr::map(data, libra::calibrateBatch.intra.rlm.single.feature, intensity= !! intensity))

  data_n_intra <- data_n %>%
    dplyr::mutate(intra_batch_center = data_intra_calibrate %>% unlist %>% mean) %>%
    dplyr::ungroup()
  ...
  data_n_intra_inter <- data_n_intra %>%
    ...
    tidyr::unnest(cols = c(data, data_intra_calibrate)) %>% dplyr::mutate(intensity_intra_inter_calibrated = intensity_intra_calibrated / factor)

  data_n_intra_inter_output <- data_n_intra_inter %>%
    dplyr::select(-intensity_intra_calibrated, -intra_batch_center, -multi_batch_center, -factor) %>% dplyr::arrange(feature, batch, injection_sequence)
